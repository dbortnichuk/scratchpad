...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def go(n: Int, pp: Int, p: Int): Int =
      if (n <= 1) p // or use "p" if not to include zero as first seq item
      else go(n-1, p, pp + p)
...
object PolymorphicFunctions {
  ...
  def main(args: Array[String]): Unit = {
    println("binarySearch: " + binarySearch(Array(1,3,4,5,6), 5, (a: Int, b: Int) => a > b))
    println("isSorted: " + isSorted(Array(1, 2, 3, 4), (a: Int, b: Int) => a < b))
    println("curry: " + curry((a: Int, b: Int) => a + b)(1)(2))
    println("uncurry: " + uncurry((a: Int) => (b: Int) => a + b)(1, 2))
    println("uncurry-curry: " + uncurry(curry((a: Int, b: Int) => a + b))(1, 2))
    println("compose: " + compose((a: Int) => a + 1, (s: String) => s.length)("hello"))
  }
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @annotation.tailrec
    def loop(idx1: Int, idx2: Int): Boolean = {
      if(idx2 >= as.length) true
      else {
        if(!gt(as(idx1), as(idx2))) false
        else {
          loop(idx2, idx2 + 1)
        }
      }
